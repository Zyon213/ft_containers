...
{
	{
		...
		for (vit = vec.begin(); vit != vec.end(); ++vit)
			map8.insert(std::pair<int, int>(*vit, *vit));
		...
		std::map<char, int>::reverse_iterator rit = map1.rbegin();
		...
		mapi.insert(std::pair<char, int>('z', 54));
		mapi.insert(std::pair<char, int>('t', 78));
		mapi.insert(std::pair<char, int>('y', 12));
		mapi.insert(std::pair<char, int>('n', 96));
		mapi.insert(std::pair<char, int>('u', 214));
		std::cout << mapi;

		std::pair<std::map<char, int>::iterator, bool> pr;

		pr = mapi.insert(std::pair<char, int>('n', 12));
		if (pr.second == false)
			std::cout << "Element '"<< pr.first->first<< "' exist with value " << pr.first->second << std::endl;
		
		std::cout << "*****Insert Element with iterator position*****\n";
		std::map<int, std::string> mapi1;
		std::map<int, std::string>::iterator itb = mapi1.begin();

		mapi1.insert(itb, std::pair<int, std::string>(32, "one"));
		mapi1.insert(itb, std::pair<int, std::string>(6, "two"));
		mapi1.insert(itb, std::pair<int, std::string>(54, "three"));
		mapi1.insert(itb, std::pair<int, std::string>(81, "seven"));
		mapi1.insert(itb, std::pair<int, std::string>(11, "nine"));
		mapi1.insert(itb, std::pair<int, std::string>(28, "ten"));
		mapi1.insert(itb, std::pair<int, std::string>(67, "eight"));
		std::cout << mapi1;

		std::cout << "*****Insert Element with iterator *****\n";
		std::map<int, std::string> mapi2(mapi1.begin(), mapi1.find(81));
		std::cout << mapi2;
		...
		for (std::map<int, int>::iterator x = smap1.begin(); x != smap1.end(); ++x)
			std::cout << x->first << " -> " << x->second << " ";
		std::cout << std::endl;
		smap2.swap(smap1);
		std::cout << "****** After swap *******\n";
		for (std::map<int, int>::iterator y = smap1.begin(); y != smap1.end(); ++y)
		...
		cmap['o'] = 54;
		cmap['f'] = 32;
		std::map<char, int>::key_compare kcmp = cmap.key_comp();
		std::map<char, int>::iterator cit = cmap.begin();
		...
		std::pair<std::map<char, int>::iterator, std::map<char, int>::iterator> pr1;
